...
import S3FileUpload, { type S3UploadedFile } from '@/components/S3FileUpload';
import SmartDocumentPane from '@/components/documents/SmartDocumentPane';
...
export default function ChatInterface({
  ...
  initialSessionId,
  onNewMessage,
}: ChatInterfaceProps) {  const { user, isAuthenticated } = useAuth();
  ...
  const [isDocumentPaneMinimized, setIsDocumentPaneMinimized] = useState(false);
  const [documentPaneSplitRatio, setDocumentPaneSplitRatio] = useState(0.6);
  const [currentSelectedAgent, setCurrentSelectedAgent] = useState<AgentType>(selectedAgent || 'health-assistant');
  ...
  // Sync selectedAgent prop with internal state
  useEffect(() => {
    if (selectedAgent && selectedAgent !== currentSelectedAgent) {
      setCurrentSelectedAgent(selectedAgent);
    }
  }, [selectedAgent, currentSelectedAgent]);

  // Show/hide document pane based on selected agent
  useEffect(() => {
    setShowDocumentPane(currentSelectedAgent === 'document-agent');
  ...
  /**
   * Save uploaded files to document database after S3 upload
   */
  const saveFilesToDatabase = async (files: S3UploadedFile[]) => {
    try {
      for (const file of files) {
        if (!file.s3Url) continue;

        // Create document record matching the expected format
        const document = {
          id: file.id,
          name: file.name,
          size: file.size,
          ...
          // Generate minimal AI analysis for compatibility
          aiAnalysis: {
            category: 'informational',
            confidence: 0.5,
            summary: 'Document uploaded from chat interface'
          },
          smartTags: []
        };

        // Extract S3 key from URL
        const s3Key = file.s3Url.split('/').slice(-1)[0];
        ...
        if (!response.ok) {
          console.error('Failed to save document to database:', response.statusText);
        } else {
          const result = await response.json();
          console.log('✅ Document saved to database:', result.document?.documentId);
          
          // Update the file with the database document ID
          file.id = result.document?.documentId || file.id;
        }
      }
    } catch (error) {
  ...
  const handleDocumentSelect = (document: any) => {
    ...
    const documentMessage = `I'd like to discuss my document: ${document.name}`;
    setInputMessage(documentMessage);
  };

  const handleDocumentUpload = (documents: any[]) => {
    if (documents.length > 0) {
      const uploadMessage = `I've uploaded ${documents.length} document(s) for analysis. Please help me understand the content.`;
      setInputMessage(uploadMessage);
    }
  };

  const handleToggleDocumentPaneMinimize = () => {
    setIsDocumentPaneMinimized(!isDocumentPaneMinimized);
  };

  const handleDocumentPaneSplitChange = (ratio: number) => {
    setDocumentPaneSplitRatio(ratio);
  };
  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      {showDocumentPane ? (
        // When document pane is visible, use ResizableSplitter
        <ResizableSplitter
          defaultSplitRatio={documentPaneSplitRatio}
          onSplitChange={handleDocumentPaneSplitChange}
          isBottomMinimized={isDocumentPaneMinimized}
          minTopHeight={300}
          minBottomHeight={200}
          className="h-full"
        >
              ...
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                <div className="flex flex-wrap gap-2">
                  {Object.entries(agentInfo).map(([key, agent]) => {
                    const isSelected = currentSelectedAgent === key;
                  ...
                  <div className="relative">
                    ...
                    {showFileUpload && (
                      <div className="absolute bottom-full mb-2 left-0 z-50">
                        <S3FileUpload
                          onFilesUploaded={(files: S3UploadedFile[]) => {
                            // Files are now uploaded to S3, we need to save them to database
          ...
          {/* Bottom Pane: Smart Document Pane */}
          <SmartDocumentPane
            isVisible={showDocumentPane}
            onClose={() => setShowDocumentPane(false)}
            onDocumentSelect={handleDocumentSelect}
            onUploadComplete={handleDocumentUpload}
            isMinimized={isDocumentPaneMinimized}
            onToggleMinimize={handleToggleDocumentPaneMinimize}
          />
        </ResizableSplitter>
      ) : (
        // When document pane is not visible, use regular layout
        <div className="flex flex-col h-full">
          {/* Agent selector - only show if showAgentSelector is true */}
          {showAgentSelector && (
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
